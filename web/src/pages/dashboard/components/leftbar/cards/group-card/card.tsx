        ...
        <ContextMenu>
            ...
            <ContextMenuTrigger>
                <div className={cn("group/card flex items-center justify-between p-1 mb-1 rounded-lg transition-colors", isActive ? "bg-accent text-accent-foreground" : "bg-card text-card-foreground hover:bg-accent/50")}>
                    ...
                    <GroupDropdown onOpen={handleOpen} data={data} onEdit={() => setIsUpdateDialogOpen(true)} onDelete={() => setIsDeleteDialogOpen(true)} />

                </div>
            </ContextMenuTrigger>

            <ContextMenuContent className="w-56">
                <GroupMenuItems onOpen={handleOpen} data={data} onEdit={() => setIsUpdateDialogOpen(true)} onDelete={() => setIsDeleteDialogOpen(true)} isContext />
            </ContextMenuContent>
